...
`cl100k_base` encoding — the same tokenizer the GPT-4o family uses — so
chunk boundaries align with what the LLM will eventually see.
...
Why a sliding window (not sentence- or paragraph-level)?
  ...
  - Overlap (CHUNK_OVERLAP_TOKENS) gives the next chunk enough leading
    context that a sentence cut in half by the window still appears whole
    in one of the two neighbors -> retrieval does not lose facts at borders.

The dataclass is named TextChunk to avoid confusion with the ORM `Chunk`
model (which this feeds into during ingestion).
"""
...
# Tunables for dense retrieval over short user-uploaded documents.
...
# for encode/decode (immutable BPE tables).
_ENCODER = tiktoken.get_encoding(_ENCODING_NAME)


@dataclass
class TextChunk:
    """One unit of text that will be embedded and stored as a Chunk row.

    `chunk_index` is the per-document ordinal (0, 1, 2, ...). The DB has
    UNIQUE(document_id, chunk_index) so a retry of ingestion overwrites
    the same rows deterministically instead of duplicating.
    """
...
def chunk_text(
    ...
    *,
    ...
    while start < total:
        ...
        # Tail guard: when the very last window is mostly overlap with the
        # previous chunk, the new tokens it contributes might be below
        # MIN_CHUNK_TOKENS. Skip it — its content already lives in the
        # previous chunk.
