...
Wraps `AsyncOpenAI` so the rest of the service stays vendor-agnostic and
async-friendly. Used during ingestion: chunker outputs N chunks →
embed_batch(N texts) → the resulting vectors are stored next to each
chunk row. (Query-time embed_one() lands in M5 with retrieval.)

Why batch? The OpenAI embeddings endpoint accepts up to 2048 inputs per
request. One-at-a-time would be ~100x slower and burn the per-minute
...
# 2048 inputs but each counts against the per-minute token quota; 100
# keeps memory + retry blast-radius small while still ~100x faster than
# one-at-a-time.
EMBED_BATCH_SIZE = 100

# SDK-level retry/backoff. Handles 429 (rate limit) and 5xx automatically
# with exponential backoff. Set once on the singleton client.
_SDK_MAX_RETRIES = 3
...
async def embed_batch(texts: list[str]) -> list[list[float]]:
    ...
    Splits into EMBED_BATCH_SIZE chunks under the hood, calls the API once
    per batch, then re-concatenates. The OpenAI SDK guarantees
    response.data is returned in the same order as the request input, so
    order is safe.
    """
    if not texts:
        return []
    ...
    for batch_start in range(0, len(texts), EMBED_BATCH_SIZE):
        ...
        # Defensive: surface a dim mismatch early. If openai_embedding_dim
        # is ever changed in config without re-running migrations, this
        # catches it before silently writing wrong-size vectors.
