...
export class ChatWidget extends LitElement {
  ...
  private renderPanel() {
    ...
    return html`
      <section class="cb-panel" role="dialog" aria-label=${title}>
        ...
        <div class="cb-scroll">
          ${this.loading
            ? html`<div class="cb-msg assistant">…</div>`
            : nothing}
          ${this.renderWelcome()}
        ...
        <div class="cb-composer">
          ...
          <button
            class="cb-send"
            ?disabled=${this.sending || this.loading || !this.draft.trim()}
            @click=${this.onSend}
          >
            Send
          </button>
        ...
        ${suggestions.length
          ? html`
              <div class="cb-suggestions">
                ${suggestions.map(
                  (q) => html`
                    <button class="cb-chip" @click=${() => this.send(q)}>
                      ${q}
                    </button>
                  `,
                )}
  ...
  private renderMessage(m: ChatMessage) {
    return html`
      <div class="cb-msg ${m.role} ${m.streaming ? "cb-cursor" : ""}">
        ${m.content}${this.renderSources(m)}
      </div>
    `;
  }
  ...
  private async ensureSession(): Promise<void> {
    ...
    try {
      ...
      this.style.setProperty(
        "--cb-primary",
        session.config.primary_color || DEFAULT_PRIMARY,
      );
    } catch {
